...
def init_db():
    with get_conn() as conn:
        ...
        if row and row[0] == 0:
            ...
            VALUES (?, ?, ?, ?, ?, 1, ?)
            """, ("ADMIN001", "Default Admin", "admin@fortrust.local", hash_password("admin123"), "ADMIN", datetime.utcnow().isoformat()))
...
def create_user(user_id: str, name: str, email: str, password: str, role: str):
    with get_conn() as conn:
        conn.execute("INSERT INTO users (user_id, name, email, password_hash, role, created_at) VALUES (?, ?, ?, ?, ?, ?)", 
                     (user_id, name, email, hash_password(password), role, datetime.utcnow().isoformat()))
        conn.commit()

def get_user_by_email(email: str):
    with get_conn() as conn:
        return conn.execute("SELECT user_id, name, email, password_hash, role, is_active FROM users WHERE email = ?", (email,)).fetchone()
    
def list_users(role: Optional[str] = None):
    with get_conn() as conn:
        if role:
            return conn.execute("SELECT user_id, name, email, password_hash, role, is_active FROM users WHERE role=? ORDER BY name", (role,)).fetchall()
        return conn.execute("SELECT user_id, name, email, password_hash, role, is_active FROM users ORDER BY role, name").fetchall()
...
# --- NEW FUNCTIONS YOU WERE MISSING ---
