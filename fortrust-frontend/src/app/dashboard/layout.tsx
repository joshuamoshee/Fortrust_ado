      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[260px] bg-[#1b1b42] text-slate-300 flex flex-col h-full shadow-2xl border-r border-[#131333] transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 py-6 px-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          {user.role === "MASTER_ADMIN" && (
            <>
              <p className="px-3 text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-3">Administrator</p>
          ...
          {/* <p className="px-3 text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-3 mt-8">Main Menu</p> */}
          ...
          {/* Generic SidebarMenu with Hover-Extended Submenu (Portal, reusable) */}
          {/* <SidebarMenu
            label="Consultation"
        ...
        <div className="p-4 bg-[#171738] border-t border-white/5">
          <div className="flex items-center justify-between bg-white/5 p-2.5 rounded-xl border border-white/5">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setShowSettings(true)}>
              <div className="w-9 h-9 rounded-full bg-linear-to-br from-[#BAD133] to-[#9bb029] flex items-center justify-center shadow-md hover:scale-105 transition-transform">
...
function SidebarMenu({ label, icon, menuItems, activePath }: SidebarMenuProps) {
  ...
  // Find the active menu item (if any)
  ...
  return (
    <>
      ...
      >
        {/* Render the icon for the main menu, highlight if any submenu is active */}
