...
import { 
  ...
  Users, MapPin, Mail, Clock, Target, AtSign, Search
} from "lucide-react";
...
export default function BroadcastHubPage() {
  ...
  const fetchAgents = async () => {
    try {
      ...
      if (data.status === "success") {
        setAgents(data.data.filter((a: any) => a.email && a.email.includes("@")));
      }
    } catch (e) {
  ...
  const filteredAgents = agents.filter(a =>
    ...
    (a.email || "").toLowerCase().includes(mentionSearch.toLowerCase())
  );

  // Preview: how many will receive this
  const targetCount = selectedAgents.length > 0
    ? selectedAgents.length
    : agents.filter(a => {
        const roleMatch = targetRole === "ALL" || a.agent_type === targetRole;
        const branchMatch = targetBranch === "ALL" || a.branch === targetBranch;
        return roleMatch && branchMatch && a.email;
      }).length;
  ...
  const handleSendBroadcast = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If specific agents selected, send individual targeted broadcast
      if (selectedAgents.length > 0) {
        // Send one broadcast per selected agent by temporarily overriding target
        // Backend handles role/branch filter — for @mention we pass names in title
        const mentionedNames = selectedAgents.map(a => `@${a.name}`).join(", ");
        const payload = {
          title: title,
          message: `${message}\n\n[Directly addressed to: ${mentionedNames}]`,
          target_role: "ALL",
          target_branch: "ALL",
          send_email: sendEmail,
          mentioned_agents: selectedAgents.map(a => a.email)
        };
        ...
        const data = await res.json();
        if (res.ok) {
          setNotification({ type: 'success', text: `Broadcast sent to ${selectedAgents.length} agent(s).` });
          setLastResult({ sent: data.emails_sent || 0, failed: data.emails_failed?.length || 0 });
          setTitle(""); setMessage(""); setSelectedAgents([]);
          fetchHistory();
        } else {
          setNotification({ type: 'error', text: data.detail || "Failed to send." });
        }
      } else {
        // Standard role/branch broadcast
        const payload = {
          title, message,
        ...
        const data = await res.json();
        if (res.ok) {
          setNotification({ type: 'success', text: data.message || "Broadcast dispatched." });
          setLastResult({ sent: data.emails_sent || 0, failed: data.emails_failed?.length || 0 });
          setTitle(""); setMessage("");
          fetchHistory();
        } else {
    ...
    <div className="p-4 lg:p-8 max-w-[1200px] mx-auto w-full relative animate-in fade-in">
        ...
        <div className={`fixed top-6 right-6 z-[100] px-6 py-4 rounded-2xl font-bold flex items-center justify-between shadow-2xl animate-in slide-in-from-top-4 max-w-md
          ...
          <div className="flex flex-col gap-1">
            ...
            {lastResult && lastResult.sent > 0 && (
              <p className="text-xs text-[#BAD133] ml-8">
                ✉️ {lastResult.sent} email{lastResult.sent !== 1 ? 's' : ''} delivered
                {lastResult.failed > 0 ? ` · ${lastResult.failed} failed` : ''}
              </p>
            )}
          </div>
          <button onClick={() => setNotification(null)} className="ml-6 opacity-70 hover:opacity-100 shrink-0"><X size={18}/></button>
        </div>
      )}
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2 font-medium text-sm">
          Push targeted announcements to agent notification bells and email inboxes.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <form onSubmit={handleSendBroadcast} className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-black text-[#282860] flex items-center gap-2">
                  <AtSign size={16} className="text-blue-500"/> Direct @ Agent (Optional)
                </h3>
              ...
              <p className="text-xs text-slate-500">Select specific agents to address directly. If selected, overrides Role/Branch filter.</p>

              {/* Selected pills */}
              {selectedAgents.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {selectedAgents.map(a => (
                    <span key={a.id} className="bg-[#282860] text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1.5">
                      @{a.name}
                      <button type="button" onClick={() => toggleAgent(a)} className="opacity-70 hover:opacity-100">
                        <X size={12}/>
                      </button>
                    </span>
                  ))}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-4 py-2 border border-slate-200 rounded-lg text-sm bg-white outline-none focus:border-[#BAD133]"
                />
              </div>
              ...
              <div className="max-h-40 overflow-y-auto space-y-1 custom-scrollbar">
                {filteredAgents.length === 0 ? (
                  <p className="text-xs text-slate-400 text-center py-3">
                    {agents.length === 0 ? "No agents with email found. Add emails to agent profiles first." : "No agents match search."}
                  </p>
                ) : (
                  filteredAgents.map(agent => {
                    const isSelected = selectedAgents.find(a => a.id === agent.id);
                    return (
                      <button key={agent.id} type="button" onClick={() => toggleAgent(agent)}
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                          isSelected ? 'bg-[#282860] text-white' : 'bg-white hover:bg-slate-50 text-slate-700 border border-slate-100'
                        }`}>
                        <div className="flex items-center gap-2 text-left">
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${isSelected ? 'bg-[#BAD133] text-[#1b1b42]' : 'bg-slate-200 text-slate-600'}`}>
                            {(agent.name || "?").charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-xs">{agent.name}</p>
                            <p className={`text-[10px] ${isSelected ? 'text-slate-300' : 'text-slate-400'}`}>{agent.email}</p>
                          </div>
                        </div>
                        {isSelected && <CheckCircle2 size={14} className="text-[#BAD133] shrink-0"/>}
                      </button>
                    );
                  })
                )}
            ...
            {/* ROLE / BRANCH TARGET — only shown when no agents selected */}
            {selectedAgents.length === 0 && (
              <div className="bg-slate-50 border border-slate-100 p-5 rounded-2xl space-y-4">
                <h3 className="text-sm font-black text-[#282860] flex items-center gap-2"><Target size={16}/> Target by Role / Branch</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Users size={12}/> Role</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><MapPin size={12}/> Branch</label>
            ...
            {/* SEND OPTIONS */}
            <div className="flex items-center justify-between border-t border-slate-100 pt-6 gap-4 flex-wrap">
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm font-bold text-slate-700 flex items-center gap-2">
                    <Mail size={16} className="text-slate-400"/> Send to email inbox
                  </span>
                </label>
                {sendEmail && (
        ...
        {/* LIVE HISTORY */}
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 flex flex-col h-full max-h-[700px]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
            {loadingHistory ? (
              ...
              <p className="text-center text-slate-400 text-sm font-medium py-10 border-2 border-dashed border-slate-100 rounded-xl">No broadcasts yet.</p>
            ) : (
              history.map(item => (
